...
class Oscillator extends AudioWorkletProcessor {
    constructor({ processorOptions: { id, notesSAB, paramsSAB, paramsLength, commSAB, notePropsLength, globalParamsStartIndex, }, }) {
        super();
        this.id = id;
        ...
        this.params = this.paramsView.subarray(this.startIndex, globalParamsStartIndex);
    ...
    calculateSlope(param, volume) {
        return param ? (-1 * volume) / Math.pow(param, 2) : 0;
    }
    calculateVolume(v, a, x, h) {
        return a * Math.pow(x - h, 2) + v;
    }
    process(_inputs, outputs) {
        const output = outputs[0];
        if (!this.params[0] || this.commView[1] === 0)
            return true;
